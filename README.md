# Топка — V20 + загрузчик фото из Яндекс Еды
...
```text
Советская ул., 1, Ставрополь
```

Добавлены ссылка на Яндекс Карты, адрес в контактах, футере и микроразметке Restaurant.

## Фото блюд

Подготовлены:

- `assets/menu/` — папка фотографий;
- `docs/MENU_PHOTO_MANIFEST.csv` — соответствие блюд и имён файлов;
- `docs/PHOTO_IMPORT.md` — инструкция;
- `tools/apply-menu-photos.py` — автоматическое подключение локальных фото.


## Автоматическое скачивание фото из Яндекс Еды

Добавлена папка:

```text
tools/yandex-photo-downloader/
```

Порядок запуска:

1. `install.bat`
2. `run_download.bat` — только скачать;
3. `run_download_and_apply.bat` — скачать и попытаться подключить к позициям меню.
